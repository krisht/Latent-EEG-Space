...
class BrainNet:
    def __init__(self, sess, input_shape=[None,  71, 125], num_output=32, num_classes=6, restore_dir=None, data='./eeg_data/'
                                                                                                          '/'):
        self.sess = sess
    ...
    def load_files(self):
        ...
        self.bckg = np.load(random.choice(self.BCKG))
        self.eybl = np.load(random.choice(self.EYBL))
        self.artf = np.load(random.choice(self.ARTF))
        self.gped = np.load(random.choice(self.GPED))
        self.pled = np.load(random.choice(self.PLED))
        self.spsw = np.load(random.choice(self.SPSW))
    ...
    def get_triplets(self):
        ...
        if choice == 'bckg':
            ii = random.randint(0, len(self.bckg['arr_0']) -1)
            a = self.bckg['arr_0'][ii]

            jj = random.randint(0, len(self.bckg['arr_0']) - 1)
            p = self.bckg['arr_0'][jj]

        elif choice == 'eybl':
            ii = random.randint(0, len(self.eybl['arr_0']) -1)
            a = self.eybl['arr_0'][ii]

            jj = random.randint(0, len(self.eybl['arr_0']) - 1)
            p = self.eybl['arr_0'][jj]

        elif choice == 'gped':
            ii = random.randint(0, len(self.gped['arr_0']) -1)
            a = self.gped['arr_0'][ii]

            jj = random.randint(0, len(self.gped['arr_0']) - 1)
            p = self.gped['arr_0'][jj]

        elif choice == 'spsw':
            ii = random.randint(0, len(self.spsw['arr_0']) -1)
            a = self.spsw['arr_0'][ii]

            jj = random.randint(0, len(self.spsw['arr_0']) - 1)
            p = self.spsw['arr_0'][jj]

        elif choice == 'pled':
            ii = random.randint(0, len(self.pled['arr_0']) -1)
            a = self.pled['arr_0'][ii]

            jj = random.randint(0, len(self.pled['arr_0']) - 1)
            p = self.pled['arr_0'][jj]

        else:
            ii = random.randint(0, len(self.artf['arr_0']) -1)
            a = self.artf['arr_0'][ii]

            jj = random.randint(0, len(self.artf['arr_0']) - 1)
            p = self.artf['arr_0'][jj]

        neg_choice = random.choice(neg_choices)

        if neg_choice == 'bckg':
            ii = random.randint(0, len(self.bckg['arr_0']) -1)
            n = self.bckg['arr_0'][ii]
        elif neg_choice == 'eybl':
            ii = random.randint(0, len(self.eybl['arr_0']) -1)
            n = self.eybl['arr_0'][ii]
        elif neg_choice == 'gped':
            ii = random.randint(0, len(self.gped['arr_0']) -1)
            n = self.gped['arr_0'][ii]
        elif neg_choice == 'spsw':
            ii = random.randint(0, len(self.spsw['arr_0']) -1)
            n = self.spsw['arr_0'][ii]
        elif neg_choice == 'pled':
            ii = random.randint(0, len(self.pled['arr_0']) -1)
            n = self.pled['arr_0'][ii]
        else:
            ii = random.randint(0, len(self.artf['arr_0']) -1)
            n = self.artf['arr_0'][ii]
...
try:
    ...
    model.train_model(learning_rate=1e-2, keep_prob = 0.5, batch_size=1000, train_epoch = 10)
